...
interface ActiveAddressChartProps {
  data: AddressMetric[]
  height?: number
}
...
function fitActivePowerLaw(data: AddressMetric[]) {
  try {
    const validData = data.filter(point => 
      point.activeAddresses > 0 && 
      !isNaN(point.activeAddresses) && 
      !isNaN(point.timestamp)
    )
    
    if (validData.length < 2) {
      throw new Error("Not enough valid data points for active addresses power law fitting")
    }
    ...
    const logY = validData.map(point => Math.log(point.activeAddresses))
    ...
    console.error('Active addresses power law fitting error:', error)
    return null
  }
}

export default function ActiveAddressChart({ data, height = 400 }: ActiveAddressChartProps) {
  ...
  const validData = useMemo(() => {
    ...
    return data.filter(point => 
      ...
      typeof point.activeAddresses === 'number' &&
      !isNaN(point.timestamp) && 
      !isNaN(point.activeAddresses) &&
      point.activeAddresses > 0 &&
      point.timestamp > 0
  ...
  // Calculate power law regression for active addresses
  const powerLawData = useMemo(() => {
    if (showPowerLaw === 'Hide' || validData.length < 10) return null
    
    return fitActivePowerLaw(validData)
  }, [validData, showPowerLaw])

  // Generate active addresses projection
  const activeProjection = useMemo(() => {
    ...
    try {
      ...
      const projectionDays = 1000
      
      const projectionPoints: Array<{timestamp: number, active: number}> = []
      
      for (let i = 1; i <= projectionDays; i += 10) {
        const futureDays = lastDays + i
        const futureTimestamp = lastPoint.timestamp + (i * 24 * 60 * 60 * 1000)
        const projectedActive = powerLawData.a * Math.pow(futureDays, powerLawData.b)
      ...
      console.error('Active projection calculation failed:', error)
      return null
    }
  ...
  const athData = useMemo(() => {
    ...
    return filteredData.reduce((max, point) => 
      point.activeAddresses > max.activeAddresses ? point : max
    )
  }, [filteredData])
  ...
  const plotlyData = useMemo(() => {
    ...
    try {
      ...
      const yValues = filteredData.map(d => d.activeAddresses)

      // Main active addresses trace
      traces.push({
        x: xValues,
        y: yValues,
        mode: 'lines',
          ...
          if (filteredIndices.length > 0) {
            ...
            if (fitX.length === viewYFit.length && viewYFit.every(y => isFinite(y))) {
              traces.push({
                ...
                mode: 'lines',
                type: 'scatter',
                name: `Active Power Law (R² ${powerLawData.r2.toFixed(2)})`,
      ...
      if (activeProjection) {
        try {
          ...
          if (timeScale === 'Log') {
            projectionX = activeProjection.map(p => getDaysFromGenesis(p.timestamp))
          } else {
            projectionX = activeProjection.map(p => new Date(p.timestamp))
          }
          
          const activeProjectionY = activeProjection.map(p => p.active)
      ...
      if (athData) {
        try {
          ...
          traces.push({
            ...
            showlegend: true,
            hovertemplate: `<b>Peak Active</b><br>Count: ${formatNumber(athData.activeAddresses)}<br>Date: ${new Date(athData.timestamp).toLocaleDateString()}<extra></extra>`,
          })
        } catch (error) {
    ...
    return traces
  }, [filteredData, timeScale, addressScale, powerLawData, activeProjection, athData, validData])
    ...
    return (
      <div className="flex items-center justify-center h-96 bg-[#0F0F1A] rounded-lg border border-gray-700">
        <div className="text-center">
          ...
          <p className="text-yellow-400 font-semibold">No Active Address Data</p>
          <p className="text-gray-400 text-sm mt-2">No valid active address data points found</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2 items-center justify-between">
        <div className="flex flex-wrap gap-2">
          <select 
            ...
            className="bg-[#1A1A2E] text-white px-3 py-1.5 rounded-md text-sm border border-gray-600 focus:border-[#F59E0B] outline-none"
          >
      ...
      <div className="text-sm text-gray-400 text-center">
        <p>
          Active Addresses: {filteredData.length.toLocaleString()} data points
          {powerLawData && ` | Power Law R² = ${powerLawData.r2.toFixed(3)}`}
          {activeProjection && showProjection === 'Show' && ` | 1000-day projection enabled`}
        </p>
      </div>
    </div>
  )
